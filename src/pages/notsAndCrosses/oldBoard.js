  ...
  const turn = (event) => {
    ...
    if (player1) {
      if (event.target.innerHTML === "") {
        ...
        else if(parseInt(event.target.id) + 2 <= 8 && parseInt(event.target.id) + 4 <= 8) {  
          console.log("checking bottom right diagonals")

        } else if(parseInt(event.target.id) + 4 <= 8 && parseInt(event.target.id) + 8 <= 8) {  
          console.log("checking bottom left diagonals")
